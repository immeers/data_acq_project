...
Below is the code for this model. The predictors are: departure airport, arrival time, and day of week.
...
There are three key takeaways from this model. Overall, we see a balanced accuracy of 73%, meaning that the predictors we chose are good predictors of having a delay.
The sensitivity and specificity values also reveal important model characteristics. The 'positive' class in our model is 0 (no delay), so that is the class to which our sensitivity score explains. Our model's sensitivity is 0.7279, meaning that the model correctly identified 73% of cases where there was no delay. The other 27% of no-delay cases were incorrectly labeled as delay. Our model's specificity is .73 as well, meaning that the model correctly identified 73% of cases where there was a delay. The other 26% of delay cases were misclassified as no delay. 

Because our sensitivity and specificity values were almost exactly alike, our model was fairly good at predicting both classes of delay. It was not skewed toward one factor or another; i.e. this model could be useful in predicting whether a flight will be delayed.

#### Linear Models
We then ran some linear models to predict the wait time once at Chicago O'Hare airport (after landing). 
The predictors for all models were: day of week, arrival time, and in-gate variation (from original expected time of arrival). The outcome variables for the 3 models were: average wait time, US citizen average wait time, and Non-US citizen average wait time.
...
## Discussion 
Analyze the results in the context of the research question or problem that the data was intended to address. Discuss any limitations of the data, such as potential sources of bias or confounding variables.

## Conclusion and Future Work
Summarize the main findings of the data acquisition process and their implications. Provide recommendations for future data collection or analysis if appropriate.

## References
Include a list of any references cited in the report, such as research papers, technical reports, websites, API documentation, news articles, etc. that informed the data acquisition process or any part of your project.

## Appendices
Include any additional information that is relevant to the project but was not included in the main report, such as extra graphs or tables.