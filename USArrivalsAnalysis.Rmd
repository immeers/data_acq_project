...
```{r Historical Flight Arrivals, eval = FALSE}
library(httr)
library(jsonlite)
...
flight_data <- read.csv('request1.csv')
head(flight_data)
```
...
flight_data$arrHour <- as.numeric(format(strptime(flight_data$arrTime, format = "%H:%M"), "%H"))
ggplot(flight_data, aes(x=arrHour)) + geom_bar() +
  labs(title = 'Arrival Times (Hour)', x = 'Hour', y = 'Count') + theme_minimal()

# Plot of most popular airports
ggplot(flight_data, aes(x=arrAirport)) + geom_bar() +
  labs(title = 'Airports Used', x = 'Airport', y = 'Count') + theme_minimal()
```
...
per_date <- wait_data %>% group_by(Date) %>% summarise(avg_us =  mean(US_Average_Wait_Time), avg_nonus = mean(Non_US_Average_Wait_Times))


us <- ggplot(per_date, aes(x = Date)) +
  geom_col(aes(y = avg_us)) +
  labs(title = "Avg Wait Times for US", x = "Date", y = "Avg Wait Time") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

nonus <- ggplot(per_date, aes(x = Date)) +
  geom_col(aes(y = avg_nonus)) +
  labs(title = "Avg Wait Times for Non US", x = "Date", y = "Avg Wait Time") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

grid.arrange(us, nonus, nrow =2)
```
