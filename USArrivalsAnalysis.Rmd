...
In our first wait model, we predict overall average wait time (for both US and Non-US citizens). Our model has an adjusted r-squared value of 0.4453, meaning that this model is not the greatest in terms of predicting overall wait time. Our model has two significant predictors: day of week and arrival time. 
  ...
  - Because day of week is a categorical variable, we can analyze the significance of the different levels. Sunday, Monday, and Wednesday are significant at the highest level (0.001). All have a positive relationship with wait time. If the flight occurs on a Sunday, the wait time increases by 3 minutes. If the flight occurs on a Monday, the wait time increases by 6 minutes. Finally, if the flight occurs on a Wednesday, the wait time increases by 24 minutes.
